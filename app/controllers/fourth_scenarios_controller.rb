class FourthScenariosController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @fourth_scenario.save
        ...
        @result = Result.create(first_scenario_id: @fourth_scenario.id, second_scenario_id: @fourth_scenario.id, third_scenario_id: @fourth_scenario.id, fourth_scenario_id: @fourth_scenario.id, user_id: 1)

        @result.save

        format.html { redirect_to edit_result_path(@result), notice: 'Fourth scenario was successfully created.' }
  ...
  private
    ...
    def fourth_scenario_params
      params.require(:fourth_scenario).permit(:ami_cc_only, :ami_cc_plus, :ami_reg_med, :ami_invasive, :ami_respirator, :ami_dialysis, :ami_cpr, :bi_cc_only, :bi_cc_plus, :bi_reg_med, :bi_invasive, :bi_respirator, :bi_dialysis, :bi_cpr, :not_sure)
    end
end
